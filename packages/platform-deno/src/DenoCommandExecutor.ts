    ...
    switch (command._tag) {
      case "StandardCommand": {
        const spawn = Effect.flatMap(Deferred.make<ExitCode>(), (exitCode) =>
          Effect.async<
            readonly [ChildProcess.ChildProcess, ExitCodeDeferred],
            Error.PlatformError
          >((resume) => {
        ...
        return pipe(
          ...
          Effect.map(([handle, exitCodeDeferred]): CommandExecutor.Process => {
            let stdin: Sink.Sink<void, unknown, never, Error.PlatformError> =
              Sink.drain;
              ...
              Effect.suspend(() =>
                handle.kill(signal)
                  ...
                  : Effect.fail(
                      toPlatformError(
                        "kill",
                        new globalThis.Error("Failed to kill process"),
                        command,
                      ),
                    ),
              );
            ...
            let stdout: Stream.Stream<Uint8Array, Error.PlatformError> =
              fromReadable<Error.PlatformError, Uint8Array>(
                () => handle.stdout!,
                (err) =>
                  toPlatformError(
                    "fromReadable(stdout)",
